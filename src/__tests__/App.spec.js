...
const factory = createFactory(({ props, store, options }) => mount(App, {
  propsData: { ...props },
  store,
  ...options,
  localVue
...
describe('Testing Fun-recipes app', () => {
  ...
  it('should show correct category information', async () => {
    ...
    expect(store.state.meal.strCategory).toBe(meal.strCategory);
    wrapper.destroy();
  });
